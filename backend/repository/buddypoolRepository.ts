...
class BuddyPoolRepository {
    ...
    async emergencyBuddyMatch(mood: EDailyMood, requiredChatCount: number): Promise<{
        userid: number;
        mood: EDailyMood;
        chatCount: number
    } | null> {
        const [rows]: any = await pool.execute(
            `SELECT userid, mood, chatCount
             FROM buddypool
             WHERE mood = ?
               AND chatCount < ?
             ORDER BY chatCount ASC LIMIT 1`,
    ...
    async incrementChatCount(userId: number): Promise<void> {
        await pool.execute(`UPDATE buddypool
    ...
    async addUser(userId: number, mood: EDailyMood): Promise<void> {
        ...
        const sql = `
            ...
            UPDATE mood =
            VALUES (mood), chatCount = 0, pronouns = ?, greenstreak = ?, dateOfBirth = ?
        `;
    ...
    */
    async findBestGreenBuddy(
        ...
        const [rows]: any = await pool.execute(
            `SELECT userid, mood, chatCount, pronouns, greenstreak, dateOfBirth
             FROM buddypool
             WHERE mood = ?
               AND chatCount < ?`,
