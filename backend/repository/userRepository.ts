  ...
  async deleteUser(userid: number): Promise<void> {
    ...
    async deleteUserAccountAndCleanup(
        ...
        try {
            ...
            await conn.execute(`UPDATE messages SET sender = ? WHERE sender = ?`, [fakeUserId, userid]);

            // 2) If user was group admin anywhere, point at fake user
            await conn.execute(`UPDATE chatdata SET admin = ? WHERE admin = ?`, [fakeUserId, userid]);

            // 3) Replace chat with fake user
            const [chatRows]: any = await conn.execute(
                `SELECT chatid FROM chatmembers WHERE userid = ?`,
                [userid]
            );
            ...
            for (const chatId of chatIds) {
                ...
                if ((fakeRows ?? []).length > 0) {
                    ...
                    await conn.execute(
                        `UPDATE chatmembers SET userid = ? WHERE chatid = ? AND userid = ?`,
            ...
            await conn.execute(`DELETE FROM scheduledmessages WHERE userid = ?`, [userid]);
            await conn.execute(`DELETE FROM moodhistory WHERE userid = ?`, [userid]);
            await conn.execute(`DELETE FROM contacts WHERE userid1 = ? OR userid2 = ?`, [userid, userid]);
            await conn.execute(
                `DELETE FROM contactrequests WHERE useridReciever = ? OR useridRequester = ?`,
                [userid, userid]
            );
   ...
   HAVING SUM(CASE WHEN userid NOT IN (?, ?) THEN 1 ELSE 0 END) = 0`,
            ...
            if (chatIdsToDelete.length > 0) {
                ...
                await conn.query(`DELETE FROM chatmembers WHERE chatid IN (?)`, [chatIdsToDelete]);
                ...
                await conn.query(`DELETE FROM chatdata WHERE chatId IN (?)`, [chatIdsToDelete]);
            }
